...
- [x] 8.3 アセンブリページとフィルタUIの相互移動
  ...
  - **実装完了 (2025-10-29)**: アセンブリページのナビゲーションボタンを追加し、クエリパラメータを保持した状態で `/parts-list` に遷移する仕組みを導入。`goto` の遷移失敗時は構造化ログで検知可能なよう `logger.error` を送出し、状態同期を保証。
