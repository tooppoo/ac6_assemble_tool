...
- [ ] 3.2 スロット切替時の条件引き継ぎロジックを実装
  - 新スロットで有効な条件のみを自動的に引き継ぐ
  - 無効化された条件を検出し、ユーザーに通知
  - スロット切替後のフィルタ済みパーツを再計算
  - 0件になった場合でも操作を継続可能にする
  - _Requirements: 1.3, 2.5, 9.1, 9.2, 9.3, 9.4_

- [ ] 4. フィルタリング機能を実装
- [ ] 4.1 既存のフィルタリングロジックを再利用
  - PartsFilterSetクラスを活用したフィルタ適用処理を実装
