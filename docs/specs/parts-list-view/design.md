...
- パーツ一覧ページ専用に再構成したフィルタリングロジック（`packages/web/src/lib/view/parts-list/filter`配下）はアセンブリページから切り離され、アセンブリページではフィルタ機能を提供しない
...
- **CompressionStream API**（ブラウザ/Node.js標準機能）: URL圧縮
  - **導入理由**: スロットごとの独立フィルタ管理（Requirement 2.5）で多数のフィルタ条件をURL共有した際のサイズ増大を抑える。追加ライブラリに頼らず標準APIでGzip圧縮を行い、URL安全なBase64に変換する
  - **代替案**: lz-string（却下理由: サードパーティ依存・パフォーマンス/互換性面でCompressionStreamが優位）
  - **セキュリティ**: クライアントサイドのみで利用し、機微情報は含めない
  - **ライセンス**: なし（標準API）
