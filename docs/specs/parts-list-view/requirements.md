...
- **manufacture（メーカー）**
- **category（カテゴリ）**
- その他 `attributes.ts` で valueType が `array` の属性

**UI仕様**:

- 属性選択ドロップダウン + 値の複数選択（チェックボックスリスト）
- 選択された値のいずれかに該当するパーツを表示（OR条件）
- 未選択の場合はすべての値を対象とする
- **i18n対応**: 値は属性に応じて翻訳またはそのまま表示（例: manufacture はメーカー翻訳、category はカテゴリ翻訳）

**選択肢**: 現在選択されているスロットのパーツに存在する候補値のみ提示
