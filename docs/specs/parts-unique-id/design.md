...
**目的**: パーツ追加・削除時のURL共有およびストレージデータの互換性を維持する。

**利用者**: AC6プレイヤー（機体構成の共有・保存）、パーツデータ管理者（パッチバージョン追加時のパーツ管理）。

**影響**: 既存のv1形式（インデックスベース）からv2形式（IDベース）への移行が必要。v1形式のURL・ストレージデータは自動変換により互換性を維持する。
...
**エラートラッキング**: 構造化ログ（JSON形式）でエラー情報を出力。機微情報（ユーザーID等）は除外。
...
**ヘルスモニタリング**: `/health`エンドポイント（既存）で稼働状況を確認。ID検証失敗時は起動を中断するため、ヘルスチェックは実行されない。
...
**XSS対策**: 機体名・説明はSvelteの自動エスケープにより保護。追加対策不要。

**アクセス制御**: IndexedDBはオリジン単位で分離。他オリジンからのアクセス不可。
