...
- [ ] 5. IndexedDBストレージのv2形式対応と移行機能の実装
- [ ] 5.1 StoredAssemblyDtoスキーマのv2形式対応
  - Zodスキーマの正規表現をv1/v2両形式に対応
  - `assembly`フィールドがv1形式（`h=0&c=1`）とv2形式（`v=2&h=HD001`）の両方を許容
  - スキーマ検証が正しく動作することを確認
  - _Requirements: 2.5_

- [ ] 5.2 IndexedDB保存処理のv2形式対応
  - 機体構成保存時にv2形式URLシリアライザーを使用
  - `assembly`フィールドにv2形式クエリ文字列を保存
  - 既存のストレージリポジトリインターフェースを維持
  - _Requirements: 2.5_

- [ ] 5.3 IndexedDB読み込み時のv1形式自動変換
  - データ読み込み時に`v`パラメータの有無でv1/v2を判定
  - v1形式データを検出した場合、v1→v2変換を実行
  - 変換後のデータをv2形式で上書き保存
  - 次回読み込み時はv2形式として処理されることを確認
  - _Requirements: 3.4, 3.5_

- [ ] 5.4 IndexedDB全データ移行処理の実装
  - 全保存済み機体データを読み込み、v1形式データを検出
  - v1形式データを一括でv2形式へ変換し、上書き保存
  - 変換結果をログ出力（成功件数、失敗件数）
  - _Requirements: 3.5, 3.6_

- [ ] 6. パッチバージョン対応とID管理ガイドラインの整備
- [ ] 6.1 ID不変性の確保とパッチ適用フローの確立
