...
- [ ] 7.1 PartsListView + FilterPanel 統合テスト
  - スロット変更時の属性リスト更新をテスト
  - 数値型属性フィルターの適用をテスト
  - 配列型属性フィルターの適用をテスト
...
- [ ] 7.2 PartsListView + SortControl 統合テスト
  - スロット変更時の属性リスト更新をテスト
  - 数値型属性ソートの適用をテスト
  - 配列型属性ソートの適用をテスト
...
- [ ] 8.1 属性取得エラーのハンドリング
  - attributes.ts の読み込み失敗時のフォールバック処理
  - 存在しないスロットキーに対するエラーハンドリング
  - フォールバック属性（共通属性のみ）の提供
  - 構造化ログによるエラー記録
  - _Requirements: 1.4_

- [ ] 8.2 フィルター・ソート適用エラーのハンドリング
  - フィルター適用時の例外をキャッチして全パーツ表示
  - ソート適用時の例外をキャッチして元の順序で表示
  - エラー発生時のユーザーへのフィードバック
  - _Requirements: 7.2_

- [ ] 8.3 UI入力検証とバリデーション
  - 数値フィルターの入力値検証（NaN, Infinity拒否）
  - 属性名の存在チェック
  - 不正な値に対するエラーメッセージ表示
  - _Requirements: セキュリティ要件（Input Validation）_
