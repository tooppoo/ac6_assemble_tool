...
**MUST**: 日本語、セキュリティファースト、構造化ログ、型安全性、例外処理、スクリプトによる生成禁止、Svelte5 rune実装
...
- **Svelte5 rune活用**：新規に作成するSvelteコンポーネントはSvelte5のrune APIを用いて実装し、リアクティビティをrune経由で表現する
...
- **プレゼンテーション層の責務最小化**：主要な計算処理はプレーンなTypeScriptモジュールに集約し、コンポーネント側は原則イベントハンドラのみを持つ（ロジックは呼び出し・組み合わせるだけ）
...
- **単一コメント単位のコミット**：1つのレビューコメントへ対応するたびに、必ず1コミットを作成する。
- **コミットメッセージにURLを含める**：上記コミットのメッセージには、対応したレビューコメントのURLを必ず含める。
- **検証の徹底**：各レビューコメントへの対応ごとに、テスト・lint・型チェックが全て成功することを確認する（必要に応じて部分実行ではなく対象パッケージのスイートを実行する）。
