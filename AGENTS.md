...
- 設計上の決定および決定までの経緯は `docs/adr/{title}.md` に配置すること。`docs/adr` 以下のテキストは ADR(Architecture Decision Record) の一般的な形式に従うこと
- `docs/adr` 配下には設計上の決定およびその経緯を配置し、具体的な対応の方針（段階移行の方針、影響スキャンなど）は Issue の概要にまとめること
