...
describe(roundUpByRealPart.name, () => {
  ...
  test('length should be same or plus 1', () => {
    fc.assert(
      fc.property(genNumAndDigit(), ({ num, digit }) => {
        const rounded = roundUpByRealPart(digit)(num)
        const len = (x: number) => `${x}`.length

        // 位上がりのケース
        expect([len(num), len(num) + 1]).toContain(len(rounded))
      }),
    )
  })
  test('all digits other than top should be 0', () => {
    fc.assert(
      fc.property(genNumAndDigit(), ({ num, digit }) => {
        const rounded = roundUpByRealPart(digit)(num)
  ...
  describe('roundUpByRealPart examples', () => {
    const testCases = [
    ...
    testCases.forEach(({ num, digit, expected }) => {
      test(`roundUpByRealPart(${digit})(${num}) should round to ${expected}`, () => {
