...
export class StateSerializer {
	private readonly VIEW_MODE_KEY = 'ac6-parts-list-view-mode'

	/**
	 * 有効なスロット一覧
	 */
	private readonly VALID_SLOTS: ReadonlySet<CandidatesKey> = new Set([
		'head',
		'core',
		'arms',
		'legs',
	...
	serializeToURL(state: SharedState): URLSearchParams {
		...
		for (const filter of state.filters) {
			params.append('filter', `${filter.property}:${filter.operator}:${filter.value}`)
		}
	...
	deserializeFromURL(
		...
		try {
			...
			if (slotParam) {
				if (!this.VALID_SLOTS.has(slotParam as CandidatesKey)) {
			...
			for (const filterParam of filterParams) {
				const parsed = this.parseFilter(filterParam)
				if (parsed) {
					filters.push(parsed)
				} else {
					logger.warn('Invalid filter parameter skipped', { filter: filterParam })
				}
			}
			...
			if (sortParam) {
				const parsed = this.parseSort(sortParam)
				if (parsed) {
					sortKey = parsed.key
					sortOrder = parsed.order
				} else {
					logger.warn('Invalid sort parameter, using default', { sort: sortParam })
				}
			}
	 ...
	 * フィルタパラメータをパース
	 */
	private parseFilter(filterParam: string): Filter | null {
	 ...
	 * 並び替えパラメータをパース
	 */
	private parseSort(sortParam: string): { key: string; order: SortOrder } | null {
