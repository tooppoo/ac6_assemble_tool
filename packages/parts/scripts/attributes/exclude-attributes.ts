...
export function excludeAttributes(
...
): readonly ACPartsAttribute[] {
  const attributesScores = new Map<
    string,
    { count: number; valueType: InnerValueType }
  >()
  ...
  forParts(parts, (attributeName, part) => {
    ...
    const valueType: InnerValueType =
      typeof value === 'number' ? 'numeric' : 'array'
    attributesScores.set(attributeName, {
  ...
  attributesScores.forEach((score, attributeName) => {
    const candidates = Array.from(
      new Set(arrayCandidates.get(attributeName) ?? []),
    )
